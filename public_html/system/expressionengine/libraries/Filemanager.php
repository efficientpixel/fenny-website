...
class Filemanager {
	...
	public function _file_datasource($state, $params)
	{
		...
		$sync_files_url = (REQ == "CP") ?
			ee()->cp->masked_url('https://docs.expressionengine.com/v2/cp/content/files/sync_files.html') :
			'https://docs.expressionengine.com/v2/cp/content/files/sync_files.html';
	...
	public function get_thumb($file, $directory_id, $ignore_site_id = FALSE)
	{
		...
		{
			if ( ! file_exists($directory['server_path'] . rawurldecode($file)))
			{
				return $thumb_info;
			}
			...
			$file = array(
				'file_name' => $file,
				'mime_type' => ee()->mime_type->ofFile($directory['server_path'] .rawurldecode($file))
			);
		}
		...
		if ($this->is_image($file['mime_type']))
		{
			...
			$thumb_info['thumb_path'] = $directory['server_path'] . '_thumbs/' .rawurldecode( $file['file_name']);
	...
	private function _browser_get_files($dir, $limit = 15, $offset = 0)
	{
		...
		foreach ($files as &$file)
		{
			...
			$file['title'] = htmlentities($file['title']);

			// We don't need this in the browser listing, unset it to avoid any
			// special character issues with json_encode
			unset($file['rel_path']);
