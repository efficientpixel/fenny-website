...
class Mo_variables_ext
{
	...
	protected function member_settings()
	{
		$member_id = $this->EE->session->userdata('member_id');
		$this->EE->db->select('m_field_id_1, m_field_id_2, m_field_id_3, m_field_id_4');
		$this->EE->db->from('member_data');
		$this->EE->db->where('member_id', $member_id);
		$query = $this->EE->db->get();
		$this->set_global_var('m_access_invoices', $query->row('m_field_id_1'));
		$this->set_global_var('m_access_dev', $query->row('m_field_id_2'));
		$this->set_global_var('m_team_id', $query->row('m_field_id_3'));
		$this->set_global_var('m_drive_letter', $query->row('m_field_id_4'));
	}
