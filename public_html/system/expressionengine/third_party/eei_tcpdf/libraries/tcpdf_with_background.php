<?php if(!defined('BASEPATH')) exit('No direct script access allowed');
  ...
  */
  ...
  /**
   * TCPDF_WITH_BACKGROUND Class
   *
   * @package      EEI_Tcpdf
   * @category     Class extension
   * @author       Massimiliano Lombardi <eei.tcpdf@age38dev.com>
   * @copyright    Copyright (c) 2011, Massimiliano Lombardi
   * @licence      http://creativecommons.org/licenses/by-nd/3.0/
   * @link         http://bitbucket.org/age38dev/eei_tcpdf
   * @note         Original code get from here: http://www.tcpdf.org/examples/example_051.phps
   * @todo         New TCPDF class must be monitored if TCPDF::Image() is changed 
   */
  class TCPDF_WITH_BACKGROUND extends TCPDF 
  {
    // --------------------------------------------------------------------
    ...
    {
      // get the current page break margin                 # original code by Nicola Asuni 
      $bMargin = $this->getBreakMargin();                  # http://www.tcpdf.org/examples/example_051.phps
      // get current auto-page-break mode                  # 
      $auto_page_break = $this->AutoPageBreak;             # 
      // disable auto-page-break                           # 
      $this->SetAutoPageBreak(FALSE, 0);                   # 
      
      // take care about page orientation 
      // (Portrait or Landscape)
      $x = 210;
      $y = 297;
      if(PAGE_ORIENTATION_A4 == 'L')
      {
        $x = 297;
        $y = 210;
      }
      ...
      $this->Image(
        PAGE_WITH_BACKGROUND_IMG, 
        0, 0, $x, $y, 
        '', '', '', 
        FALSE, 300, '', 
        FALSE, FALSE, 0
      );
      // restore auto-page-break status                    # original code by Nicola Asuni
      $this->SetAutoPageBreak($auto_page_break, $bMargin); # http://www.tcpdf.org/examples/example_051.phps
      // set the starting point for the page content       # 
      $this->setPageMark();                                # 
      
      // check if we need page header
      if(!defined('PAGE_WITH_NO_HEADER'))
      {
        // run the original code class
        parent::Header();  
      }
    ...
    # @TODO: this have to repeated on each TCPDF/FPDI class extension (and MUST be refactor to avoid code replication) 
    public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array())
    {
      $safe_mode    = ini_get('safe_mode');
      $open_basedir = ini_get('open_basedir');
      
      if(($safe_mode == TRUE OR trim($open_basedir) != '') AND EEI_TCPDF_WORKAROUND_SAFEMODE == TRUE)
      {
        log_message('debug', '{'.__CLASS__.'::'.__FUNCTION__.'::'.__LINE__.'} Calling modified version of TCPDF::Image');
        
        ### ORIGINAL CODE MOD START ###
        {
          if ($x === '') {
              $x = $this->x;
          }
          ...
          if ($file{0} === '@') {
            ...
            if ($imsize === FALSE) {
              if (function_exists('curl_init')) {
                ...
                // @note Commenting this line solve the problem 
                //       (quick and dirty) for open_basedir AND/OR 
                //       safe_mode 
                ### curl_setopt($cs, CURLOPT_FOLLOWLOCATION, true);
                // end modification of original code
        ...
        ### ORIGINAL CODE MOD END ###   
      }
      
      log_message('debug', '{'.__CLASS__.'::'.__FUNCTION__.'::'.__LINE__.'} NO need to call modified version of TCPDF::Image');
      return parent::Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array());
  ...
  /* End of file tcpdf_with_background.php */
  /* Location: ./system/expressionengine/third_party/eei_tcpdf/libraries/tcpdf_with_background.php */
